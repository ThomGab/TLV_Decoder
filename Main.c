...
int main(int argc, char *argv[]){
	...
	for( file_pos = 0; source_file_arr[file_pos] != '\0'; file_pos++){
		...
		switch(Reading_Status){
			...
			case(1):
				...
				if( (Tag_Buffer == NULL) ){
					Tag_Buffer = malloc( ((sizeof * Tag_Buffer) * ((TagField_Size_Bytes * 2) + 1)) );
					*Tag_Buffer = '\0';
					...
					if (Is_Bit_Set(Processing_Subsequent_Field_FirstNibFlag, nibble_flags_ptr) && Is_Bit_Set(Processing_Subsequent_Field, nibble_flags_ptr)) {
						...
						Temp = realloc(Tag_Buffer, ((sizeof(char) * TagField_Size_Bytes * 2) + 1));
						if (Temp != NULL) {
							Tag_Buffer = Temp;
							Temp = NULL;
						}
				...
				/*if ((Is_Bit_Set(Processing_Tag_FirstByte, nibble_flags_ptr) == 0) && (Is_Bit_Set(Processing_Large_TagField_firstnib, nibble_flags_ptr) == 1)) {
						Tag_Buffer_Start = realloc(Tag_Buffer_Start, ((sizeof * Tag_Buffer) * ((TagField_Size_Bytes * 2) + 1)));
				...
				//strcat(Tag_Buffer, input_nibble_str);
				strcat(Tag_Buffer, input_nibble_str);
				printf("Tag_Buffer, \t %s\n", Tag_Buffer);
			...
			case(2):
				...
				Tag_Def = Find_Tag_Def(Tag_Def, Tag_Buffer, TagList);
								
				if (Tag_Def != NULL){
					//change this to write into the final TLV_Block one line at a time.
					TLV_Block = malloc( (sizeof(char)) * (strlen(Tag_Def) + strlen(Tag_Buffer) + 2) );
					strcpy(TLV_Block,Tag_Buffer);
				...
				if(Is_Bit_Set(Processing_Length, nibble_flags_ptr)){			
					...
					Temp = realloc(TLV_Block, (( (strlen(TLV_Block) + strlen("\tLength: ") + (Length_Field_Size * 2) + 1) * sizeof(char) )) );
			...
			case(3):
				...
				if (Is_Bit_Set(Processing_Length, nibble_flags_ptr)) {
					...
					Temp = realloc(TLV_Block, (((strlen(TLV_Block) + (Length_Field_Size * 2) + strlen("\tLength: ") + 1) * sizeof(char))));
			...
			case(4):
				...
				//The Length is coded across multiple Bytes.
					...
					if (Length_Field_Pos <= ((Length_Field_Size*2)) ) { //Field Position in Nibbles, Length_Field_Size is in Bytes.
						...
						Temp = realloc(TLV_Block, (((strlen(TLV_Block) + strlen("\n\tValue: ") + (Length*2)+ 1) * sizeof(char))));
						if (Temp != NULL) {
							TLV_Block = Temp;
						}
						else {
				...
				//Write to Nibble to Length TLV_Block:
	...
	if (Reading_Status != 6) {
		...
		if (Output == NULL) {
			...
			if (TLV_Block != NULL || Tag_Buffer != NULL || Tag_Def != NULL) {
				...
				if (TLV_Block != NULL) {
					...
					if (Tag_Buffer != NULL) {
						Temp = NULL;
						Temp = realloc(Output, (strlen(Output) + strlen(Tag_Buffer) + 1));
						if (Temp != NULL) {
							Output = Temp;
							strcat(Output, Tag_Buffer);
							Tag_Buffer = NULL;
							Temp = NULL;
						}
						else {
