...
int main(int argc, char* argv[]) {
	...
	char * source_file_arr_ptr = NULL;
	size_t source_file_size = 0;
	size_t source_file_arr_size = 0;
	...
	if (validate_runtime_arguments(argc, argv) != 1 || validate_file(argc, argv) != 1) { printf("Exiting...\n"); return 0; };

	printf("Opening File...\n\n");
	source_file_fp = fopen(argv[1], "r");

	source_file_size = get_file_size(source_file_fp);
	
	//Create and write the file content to an array, and then close the file.
	source_file_arr_ptr = malloc((source_file_size) * sizeof(char));

	if (source_file_arr_ptr == NULL) {
		return;
	}	

	fread(source_file_arr_ptr, sizeof(char), source_file_size, source_file_fp);
	fclose(source_file_fp);
	source_file_arr_ptr[source_file_size] = '\0';
	...
	for (current_file_pos = 0; source_file_arr_ptr[current_file_pos] != '\0'; current_file_pos++) {
		...
		input_nibble_cleaned = Clean_Input((source_file_arr_ptr[current_file_pos]));
		...
		while (ReadyforNextNibble == 0) {
			...
			case(6):
				...
				if (y == 0) {
					...
					if (Output == NULL) {
						Output = malloc(source_file_arr_size * sizeof(char));
					}
					...
					for (y = 0; y < current_file_pos; y++) {
						Output[y] = source_file_arr_ptr[y];
					}
	...
	if (Invalid_Data_Found == 1) {
		...
		Output = malloc(sizeof(char) * 500000000);
		Output[0] = '\0';
